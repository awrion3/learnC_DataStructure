...
void HInsert(Heap* ph, HData data, Priority pr) {
	...
	HeapElem nelem = { pr, data };		//새 노드의 생성 및 전달받은 데이터로 초기화

	//새 노드가 저장될 위치가 루트 노드의 위치가 아니면 계속 반복
	while (idx != 1) {
		if (pr < (ph->heapArr[GetParentIDX(idx)].pr)) {	//새 노드가 새 노드의 부모 노드보다 우선순위가 높다면
	...
	ph->heapArr[idx] = nelem;	//새 노드를 실제로 해당 위치에 저장
	ph->numOfData++;			//데이터 전체 개수 하나 증가
}

//힙의 데이터 삭제//루트 노드 비우고, 마지막 노드로 채우고, 자식 노드와의 비교를 통해 하강
HData HDelete(Heap* ph) {
	HData retData = (ph->heapArr[1]).data;			//삭제할 데이터 저장(힙의 루트 노드의 데이터 저장)
	HeapElem lastElem = ph->heapArr[ph->numOfData]; //구조체 대입 초기화(힙의 마지막 노드 저장)
	...
	while (childIdx = GetHiPriChildIDX(ph, parentIdx)) { //루트 노드의 우선순위가 높은 자식 노드에서 반복문 시작
		if (lastElem.pr <= ph->heapArr[childIdx].pr)	 //마지막 노드와 해당 자식 노드의 우선순위 비교
